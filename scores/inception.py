...
def build_inception_model(align_tf=True, transform_input=True):
    ...
    model = Inception3(num_classes=num_classes, aux_logits=False, transform_input=transform_input, align_tf=align_tf)
    ...
    def _forward(self, x):
        ...
        if self.use_max_pool:
            ...
            branch_pool = F.avg_pool2d(
                x, kernel_size=3, stride=1, padding=1, count_include_pad=self.pool_include_padding
            )
        branch_pool = self.branch_pool(branch_pool)
